#' Get the Burden variable and Burden effect size
#'
#' The \code{Gene_Centric_Coding_Burden} function performs gene-centric coding burden analysis for each gene in the given dataframe.
#'
...
#'
#' @return A list of results from gene-centric coding burden analysis for each gene.
#' @export



Gene_Centric_Coding_Burden <- function(chr, df_gene, genofile, obj_nullmodel, rare_maf_cutoff=0.01, rv_num_cutoff=2,
  ...
  for (i in 1:nrow(df_gene)) {
    results[[i]] <- Gene_Centric_Coding_Burden_each(chr=df_gene$CHR[i], gene_name=df_gene$gene_name[i], category=df_gene$category[i],
